...
question_no=0
selections = [0]*9
with open('comp_fun_questions.json') as cfqj:
    comp_fun_quiz_json = load(cfqj)
questions = (comp_fun_quiz_json['question'])
options = (comp_fun_quiz_json['options'])
answers = (comp_fun_quiz_json[ 'answer'])

def start_quiz():
    gk_window = Tk()
    gk_window.title('TCP Quiz - Computer Fundamentals Quiz Server')
    ...
    title_label = Label(title_frame, text = 'Welcome to TCP Computer Fundamentals Quiz', width = 800, bg = '#88B2CC', font=helv12, justify='center')
