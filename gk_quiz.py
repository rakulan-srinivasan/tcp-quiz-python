...
question_no=0
selections = [0]*9
with open('gk_questions.json') as gkqj:
    gk_quiz_json = load(gkqj)
questions = (gk_quiz_json['question'])
options = (gk_quiz_json['options'])
answers = (gk_quiz_json[ 'answer'])

def start_quiz():
    gk_window = Tk()
    gk_window.title('TCP Quiz - General Knowledge Quiz Server')
    ...
    title_label = Label(title_frame, text = 'Welcome to TCP General Knowledge Quiz', width = 800, bg = '#88B2CC', font=helv12, justify='center')
